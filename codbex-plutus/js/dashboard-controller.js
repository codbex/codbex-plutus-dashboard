...
dashboard.controller('DashboardController', [
	...
	'$http',
	'messageHub',
	function ($scope, $http, messageHub) {
		$scope.state = {
			isBusy: true,
			error: false,
			busyText: 'Loading...',
		...
		$http
			.get(
				'/services/js/codbex-plutus/api/WidgetsExtension/WidgetService.js'
			)
		...
		function createWidget(widgetData) {
			...
			if (widgetData.size == 'small') {
				widgetContainer.className =
					'fd-col fd-col--6 fd-col-md--3 fd-col-lg--3 fd-col-xl--3';
			} else if (widgetData.size == 'medium') {
				widgetContainer.className =
					'fd-col fd-col--12 fd-col-md--6 fd-col-lg--6 fd-col-xl--6';
			} else {
				// large - TODO: needs to be made higher.
				widgetContainer.className =
					'fd-col fd-col--12 fd-col-md--6 fd-col-lg--6 fd-col-xl--6';
			}
		...
		messageHub.onDidReceiveMessage('contextmenu', function (msg) {
			if (msg.data === 'sales-orders') {
				messageHub.postMessage(
					'launchpad.switch.perspective',
					{ perspectiveId: 'sales-orders' },
					true
				);
